...
"""Rebuild citation counters and page images from primary-source PDFs."""
...
for citation, count in entry_map["vlb"].most_common():
    ...
    for instance in instances:
        ...
        if not is_malformed:
            citations.update(
                [(*elements[:3], page if elements[1] == "i" else page + 154)]
            )
...
for citation, count in entry_map["ar."].most_common():
    instances = [
        x.strip()
        for x in citation.split(";")
        if "ar., cat" in x.lower() and "1686" not in x
    ]
    for instance in instances:
        is_malformed = False
        try:
