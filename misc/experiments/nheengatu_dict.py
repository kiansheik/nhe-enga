"""Inspect font and character runs in the Nheengatu source PDF."""
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_num, page in enumerate(pdf.pages):
            ...
            for char in page.objects.get("char", []):
                ...
                if (
                    char_text.strip()
                ):  # If it's not whitespace, add it to the current word
                    if current_font_size is None:
                        current_font_size = (
                            font_size  # Set the font size for the first char
                        )

                    # If font size changes, treat it as the end of a word
                    if font_size != current_font_size:
                        words.append(
                            {"word": current_word, "font_size": current_font_size}
                        )
