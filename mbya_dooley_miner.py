...
cid_map = {
    ...
    'AGEFET+TTE2A45118t00': {'(cid:1)': 'ĩ',
                            '(cid:2)': 'ũ',
                            '(cid:3)': 'ỹ',
                            '(cid:4)': 'ẽ'
                            },
    'FWSEET+TTE2A45898t00': {'(cid:1)': 'ĩ',
                            '(cid:2)': 'ỹ',
                            '(cid:3)': 'ẽ',
                            '(cid:4)': 'ũ'},
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in tqdm(pdf.pages):
            for elem in page.parse_objects()['char']:
                ...
                #     cid_map[font][letter] = ""
                lat = cid_map.get(font, dict()).get(letter, letter.replace("cid:", f"{font}:cid:"))
                if lat:
                    letter = lat
                # if "cid" in letter:
                #     breakpoint()
