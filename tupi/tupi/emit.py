...
def render_annotated(morphs: Iterable[Morpheme]) -> str:
    ...
    for m in morphs:
        if m.tags:
            ...
            if base_tags and op_ids and not any(
                t.startswith("OP_TYPE_") or t.startswith("OP_SCOPE_") or t.startswith("OP_MAX_ARGS_")
                for t in base_tags
            ):
...
def with_op_morphs(
    ...
    ops = [
        op_morpheme(i, ev, max_args.get(i))
        for i, ev in enumerate(em.ops or [])
    ]
