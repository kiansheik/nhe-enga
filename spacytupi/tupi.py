...
class Verb:
    ...
    def conjugate(self, subject_tense='1ps', object_tense='3p', dir_obj_raw=None, mode='indicativo', pos='posposto', pro_drop=False):
        ...
        if self.verb_class in ["(v. intr.)"]:
            ...
            if object_tense in TupiAntigo.personal_inflections.keys():
                if object_tense == subject_tense:
                    ...
                    result = f"{subj} {TupiAntigo.personal_inflections[subject_tense][2]}-îe-{self.verbete}"
                elif object_tense == '3p':
                    subj = TupiAntigo.personal_inflections[subject_tense][0] if not pro_drop else ''
                    ...
                    if pos == 'posposto':
                        ...
                        result = f"{subj} {pref}-{'î' if dir_obj_raw is None else dir_obj}-{self.verbete}"
                # elif object_tense == '1ps':
                #     if subject_tense == '1ps'
        else:
