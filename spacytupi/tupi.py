...
class TupiAntigo(spacy.language.Language):
    ...
    ipa_map = {
        "Obstruintes": "p pʷ pʲ β t s sʷ k kʷ ʔ",
...
class Verb:
    ...
    def conjugate(
        ...
        if self.verb_class in ["adj.: "]:
            result = (
                f"{TupiAntigo.personal_inflections[subject_tense][1]} {self.verbete}"
            )
        if self.verb_class in ["(v. intr.)"]:
            subj = (
                TupiAntigo.personal_inflections[subject_tense][0]
                if not pro_drop
                else ""
            )
            ...
            if object_tense in TupiAntigo.personal_inflections.keys():
                if object_tense == subject_tense:
                    subj = (
                        TupiAntigo.personal_inflections[subject_tense][1]
                        if not pro_drop
                        else ""
                    )
                    ...
                    dir_obj = (
                        TupiAntigo.personal_inflections["3p"][0]
